...
{
    if(argc < 3) {
        cout << "./VideoStab [video.avi] [out.avi]" << endl;
        return 0;
    }
    ...
    // Step 5 - Apply the new transformation to the video and writing to file
	VideoWriter vidout(argv[2],CV_FOURCC('M','J','P','G'),cap.get(CV_CAP_PROP_FPS),
					Size(cap.get(CV_CAP_PROP_FRAME_WIDTH),cap.get(CV_CAP_PROP_FRAME_HEIGHT)),true);
