...
cdef extern from "SLIC.h":
    cdef cppclass SLIC:
        ...
        # img, width, heights, depth, returned labels, num_labels, superpixel size, compactness
        void DoSupervoxelSegmentation(unsigned int ** , int ,int, int, int **, int, int, double)
        # img, labeling, width, heights, color for contours
        void DrawContoursAroundSegments(unsigned int** ,int*, int, int, unsigned int)
        
        
def slic_nv(np.ndarray[np.uint8_t, ndim=4] img, n_superpixels=500, compactness=10):
    """SLIC Supervoxels for fixed number of superpixels.

    Parameters
    ----------
    frames : numpy array, dtype=uint8
        Original image RGB.
        Needs to be C-Contiguous
    n_supervoxels: int, default=500
    ...
    cdef np.ndarray[np.uint8_t, ndim=4] img_ = np.empty((img.shape[0],img.shape[1], img.shape[2], 4), dtype=np.uint8)
    img_[:, :, :, :-1] = img
    ...
    slic.DoSupervoxelSegmentation(<unsigned int ** &>img_.data, w, h, d, labels, n_labels, n_superpixels, compactness)
    cdef np.npy_intp shape[3]
    shape[0] = d
    shape[1] = h
    shape[2] = w

    label_array = np.PyArray_SimpleNewFromData(3, shape, np.NPY_INT32, <void*> labels)
    return label_array
