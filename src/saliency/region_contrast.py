...
class RegionContrast(Saliency):
	...
	def performSaliency(self):
		...
		saliency = uniqueness#normalize(uniqueness*np.exp(-2 *distribution))#
		...
		if self.props.doProfile:
			u_frame = sum([np.where(self.regions==region,255*uniqueness[region],0) 
							for region in range(self.num_regions)],0)
			d_frame = sum([np.where(self.regions==region,255*distribution[region],0)
						for region in range(self.num_regions)],0)
