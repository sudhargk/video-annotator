...
class BlockTask(object):
	...
	def process(self,vidwriter,predictors,designFrameBaner,predictorStartIdx):
		scores = [];
		dispFrames,processed_frames = self.__buildFrames__();
		start = time.time();
		for predictor in predictors:
			scores.extend([predictor.get_score(processed_frames)]);
		print time.time()-start
		
		for idx,frame in enumerate(dispFrames):
			blockIdx = idx/self.block_size;_score = [];
			for grpClassIdx  in range(len(scores[0][blockIdx])):
				a_score = np.array(scores[grpClassIdx+1][blockIdx])*scores[0][blockIdx][grpClassIdx];
				_score.extend([a_score]);
			_score = np.hstack(_score);
			#print self.labels[blockIdx],_score.shape,np.argmax(_score),np.sum(_score);
			_frame = designFrameBaner(frame,_score,self.labels[blockIdx]);
