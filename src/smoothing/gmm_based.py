...
class GMMBased(Smoothing):
	def __init__(self,_feats, numMixtures = 4,threshold=0.6):
		super(GMMBased,self,).__init__(_feats)
		self.numMixtures =  numMixtures
		self.threshold = threshold
		
	def __build_model__(self,fg_mask_feats,bg_mask_feats):
		fg_gmm = GMM(n_components=self.numMixtures,covariance_type='diag',n_iter=3,random_state=1);
		fg_gmm.fit(fg_mask_feats);
		bg_gmm = GMM(n_components=self.numMixtures,covariance_type='diag',n_iter=3,random_state=1);
	...
	def __get_score__ (self,block,frame_feats,shape,gmm):
		...
		#fg_score = normalize(gmm[0].score(frame_feats))
		#bg_score = normalize(gmm[1].score(frame_feats))
		#fg_prob =  ((1 + fg_score - bg_score)/2).reshape((shape[0],shape[1]))
		...
		#mask_cut[fg_prob >= 0.4] = 2
		#mask_cut[fg_prob >= 0.6] = 3
		#mask_cut[fg_prob >= 0.8] = 1
		#cv2.grabCut(block,mask_cut,rect,bgdModel,fgdModel,1,cv2.GC_INIT_WITH_MASK)
