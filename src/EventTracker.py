...
def process(vidreader,vidwriter):
	...
	while(True):
		...
		lab_frame = cv2.cvtColor(bg_es.cur_frame,cv2.COLOR_RGB2LAB);
		#if np.all(count>4):
		cv2.grabCut(lab_frame,mask_cut,rect,bgdModel,fgdModel,1,cv2.GC_INIT_WITH_MASK)
		#else:
		...
		out_frame = cv2.addWeighted(np.float32(bg_es.cur_frame),0.6,mask_frame,0.4,0.0);
