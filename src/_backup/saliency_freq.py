...
class Saliency(object):	
	...
	def build_region(self,useLAB = True):
		...
		self.color_data = np.array([np.sum(self.q_frame[np.where(self.regions==region)],0)
	...
	def freq_tuning(self,weights=[0.5,0.4,0.3,0.2,0.1,0.05,0.03,0.01]):
		start_time = time.time();
		_mean = self.mean[:self.num_regions,];
		_color = self.color_data[:self.num_regions,].copy();
		_norm = np.sqrt(self.shape[0]*self.shape[0] + self.shape[1]*self.shape[1]);
		_allp_dist = squareform(pdist(_mean))
		prev_saliency = np.ones(self.num_regions,dtype=np.float)
